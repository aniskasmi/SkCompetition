      ...
      <div class="col-lg-3">
        <div class="card blog-widget">
          <div class="card-body">
            ...
            <?php if ($scores->findMultiple( 'users', 'games', $user->getId(), $game->getId())): ?>
              <a href="" class="btn btn-block btn-success waves-effect waves-light mt-1">Jeu joué</a>
            <?php else: ?>
              <a target="_blank" href="<?= $router->generate('adminEnter', ['idUser' => $user->getId(), 'idGame' => $game->getId()]); ?>" class="btn btn-block btn-danger waves-effect waves-light mt-1">Entrer le score de <?= $user->getPseudo(); ?></a>
            <?php endif; ?>
          </div>
        </div>
      </div>
